...
export const forgetPassword = async (req, res, next) => {
  try {
    const userDetails = await findUserByMailId(req.body.email);
    if (!userDetails) {
      return res.status(404).json({ success: false, message: "User not found." });
    }
    const token = await attachToken(req.body.email);
    if (!token) {
      return res.status(500).json({ success: false, message: "Error generating password reset token." });
    }

    await sendPasswordResetEmail(userDetails, token);
...
export const resetUserPassword = async (req, res, next) => {
  ...
  if (user) {
    const isPasswordReset = await resetPassordRepo(user.email, password, confirmPassword);
    if (isPasswordReset) {
...
export const updateUserProfileAndRole = async (req, res, next) => {
  ...
  try {
    ...
    const userDetails = await updateUserRoleAndProfileRepo(req.params.id,req.body );
    if (!userDetails) {
      return res
        .status(400)
        .json({ success: false, msg: "no user found with provided id" });
